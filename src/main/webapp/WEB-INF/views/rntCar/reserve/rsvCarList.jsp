		...
		$(".rsvDetail")
				.on(
						...
						function() {
							...
							$
									.ajax({
										url : "/rntCar/reserve/rsvCarListPost",
										data : {
											"rsvId" : rsvId
										},
										type : "post",
										...
										success : function(result) {
											...
											str += `<li class='list-group-item mb-3'> <span>차 종류명:</span> <span class='h6 fw-normal ms-1 mb-0'>\${result.carKnd}</span> </li>`;
											str += `<li class='list-group-item mb-3'> <span>차 번호:</span> <span class='h6 fw-normal ms-1 mb-0'>\${result.carNum}</span> </li>`;
											...
											str += `<li class='list-group-item mb-3'> <span>결제 금액:</span> <span id='pay' class='h6 fw-normal ms-1 mb-0'>\${result.payPr}원</span> </li>`;
					...
					<div class="col-12">
						...
						<div class="card shadow mt-5">
							...
							<div class="card-body">
								...
								<!-- 										페이징 , 기간검색 처리 -->
								<!-- Table body START -->
								<c:forEach var="carListVO" items="${data.content}"	varStatus="stat">
									<c:set var="formattedRsvDt">
									    <fmt:formatDate value="${carListVO.rsvDt}" pattern="yyyy/MM/dd" />
									</c:set>
									<div
										class="row row-cols-xl-7 align-items-lg-center border-bottom g-4 px-2 py-4">
									
										<div class="col" style="text-align: center;">
											<small class="d-block d-lg-none">차종류 번호</small>
											<h6 class="mb-0 fw-normal">${carListVO.carKnd}<br/>(${carListVO.carNum})</h6>
										</div>


										<div class="col" style="text-align: center;">
											<small class="d-block d-lg-none">회원명</small>
											<h6 class="mb-0 fw-normal">${carListVO.memNm}</h6>
										</div>
										
										<div class="col" style="text-align: center;">
											<small class="d-block d-lg-none">예약한날</small>
											<h6 class="mb-0 fw-normal">${formattedRsvDt}</h6>
										</div>
										<div class="col" style="text-align: center;">
											<small class="d-block d-lg-none">예약 기간:</small>
											<h6 class="mb-0 fw-normal">${carListVO.rsvPeriod}</h6>
										</div>
										
										
										<c:set var="memTel" value="${carListVO.memTel}" />
										...
										<div class="col" style="text-align: center;">
											...
											<c:if test="${carListVO.rsvCode eq '입금대기'}">
												<div class="badge bg-warning bg-opacity-10 text-warning">${carListVO.rsvCode}</div>
											</c:if>
											<c:if test="${carListVO.rsvCode eq '예약확정'}">
												<div class="badge bg-success bg-opacity-10 text-success">${carListVO.rsvCode}</div>
											</c:if>
											<c:if test="${carListVO.rsvCode eq '환불요청'}">
												<div class="badge bg-danger bg-opacity-10 text-danger">${carListVO.rsvCode}</div>
											</c:if>
											<c:if test="${carListVO.rsvCode eq '환불완료'}">
												<div class="badge bg-info bg-opacity-10 text-info">${carListVO.rsvCode}</div>
											</c:if>
										</div>

										<div class="col" style="text-align: center;">
											<a href="#" class="btn btn-sm btn-info-soft rsvDetail"
												data-toggle="modal" data-reserve-id="${carListVO.rsvId}"
									...
									<nav class="mb-sm-0 d-flex justify-content-center"
										...
										<ul
											...
											<li
												...
												href="/rntCar/reserve/rsvCarList?currentPage=${data.startPage-5}&keyword=${param.keyword}&sort=${param.sort}&period=${param.period}"
											...
											<c:forEach var="pNo" begin="${data.startPage}"
												...
												<li
													...
													<a
													href="/rntCar/reserve/rsvCarList?currentPage=${pNo}&keyword=${param.keyword}&sort=${param.sort}&period=${param.period}"
