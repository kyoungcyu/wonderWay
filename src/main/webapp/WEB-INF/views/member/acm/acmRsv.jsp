				...
				<div class="vstack gap-5">
							...
							<div class="card mb-4">
								<div class="row align-items-center">
									...
									<div class="col-sm-6 col-md-3">
										<img src="/resources/upload/busi${busiVO.busiImg}" class="card-img" alt="">
									</div>
								...
								<div class="card-body">
									...
									<ul class="list-group list-group-borderless mb-0">
										...
										<%
									        ...
									        String msg="무료 취소 가능기간이 지났습니다.";
										%>
										
										<li class="list-group-item h6 fw-light d-flex mb-0"><i class="bi bi-patch-check-fill text-success me-2"></i><span class="text-danger"><%=freeRfdDate%></span>까지 무료 취소 가능
										<%if(now.isAfter(oneMonthAgo)){%>
					...
					<div class="card shadow">
											...
											<div class="row g-3 mt-3">
												...
												<div class="col-12">
													<div class="d-sm-flex justify-content-sm-between align-items-center">
														<fmt:formatNumber var="formattedRmPr" value="${busiVO.rmVOList[0].rmPr}" pattern="#,###,###"/>
														<h4> <span class="small fs-6">최종 가격 </span> <span class="totalFee">${formattedRmPr}원</span></h4>
														<button id="pay" class="btn btn-primary mb-0"  data-placement="bottom" >결제하기</button>
													</div>
												</div>
											</div>
											<!-- Form END -->
						...
						<div class="card-footer p-4 pt-0">
							<!-- Condition link -->
							<p class="mb-0">결제시  <a href="http://localhost/member/main/termsConditions" class="text-primary">Wonderway 통합정책</a>에 동의한 것으로 간주합니다.</a></p>
						</div>
					</div>
					<!-- Payment Options END -->
				</div>	
						...
						<div class="card shadow rounded-2">
							...
							<div class="card-body py-1">
								<ul class="list-group list-group-borderless">
									<li class="list-group-item d-flex justify-content-between align-items-center">
										...
										<span class="fs-5 baseFee fee">${formattedRmPr}원</span>
									</li>
									<li class="list-group-item d-flex justify-content-between align-items-center">
										<span class="h6 fw-light mb-0">인원 추가 비용</span>
										<span class="fs-5 exFee fee">0원</span>
									</li>
									<li class="list-group-item d-flex justify-content-between align-items-center">
										<sec:authorize access="isAnonymous()">
							...
							<div class="card-footer border-top">
								<div class="d-flex justify-content-between align-items-center">
									...
									<span class="h5 mb-0 totalFee">${formattedRmPr}원</span>
								</div>
							</div>
						</div>
					</div>
...
$(function() {
	...
	$("#btnMlg").on("click",function(){
		var mlgWithCommas=numberWithCommas(-$("#mlgInput").val())
		$(".mlg").text( mlgWithCommas+"원")
		totalFee();
	...
	$("#mlgInput").on("input",function(){
	    var enteredValue = parseInt($(this).val());
		    var maxLimit = 500000${memVO.memMlg}; 
	...
	$(".cupnCard").on("click",function(){
		...
		$("#btnApplyCupn").on("click",function(){
			...
			var fee=parseInt($(".baseFee").text().replace(/,/g,'').slice(0,-1)) +parseInt($(".exFee").text().replace(/,/g,'').slice(0,-1))
			var cupnDiscount=fee*parseInt(rt)*0.01
			$(".cupnInput").val(cupnDc)
			var cupnWithCommas=numberWithCommas(-cupnDiscount)
			$(".cupn").text(cupnWithCommas+"원")
			totalFee()
		})
	...
	$('#pay').click(function(){
		...
		var amount=parseInt($(".totalFee").text().replace(/,/g,'').slice(0,-1));
...
$(function() {
	...
	$(".touchspin").on('change',function(){
		...
		if(ppNum>stdPp){
			...
			var exFeeWithCommas=numberWithCommas(totalExFee)
			$(".exFee").text(exFeeWithCommas+"원");
			totalFee();
