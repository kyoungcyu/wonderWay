...
public class TrnController{
	...
	public String searchTrn(
		    ...
		    @RequestParam(value="numPp",required=false, defaultValue="1") String numPp,
		    HttpSession session){
	...
	public String searchResultAcm(Model model, HttpSession session) throws IOException, JAXBException, ParseException{
		...
		String depPlandTimeWithSlash=(String) session.getAttribute("depPlandTime");
		String numPp=(String) session.getAttribute("numPp");
		String depPlandTime = depPlandTimeWithSlash.replaceAll("/", "");
...
//				
		...
		model.addAttribute("depPlandTime", depPlandTimeWithSlash);
	...
	public String trnRsv(@RequestParam("traingradename") String traingradename,
		...
		if(rmNum=="" && seatId=="") {
			...
			map.put("trnNum",trainno);
			map.put("trnKnd",traingradename);
			map.put("stLoc",depplacename);
			map.put("arLoc",arrplacename);
			map.put("stTm",depTime);
			map.put("arTm",arrTime);
			map.put("numPp",numPp);
			
			List<TrnSeatVO> trnSeatVOList=mmTrnService.getRndSeat(map);
			log.info("랜덤좌석"+trnSeatVOList);
			rmNum=trnSeatVOList.get(0).getRmNum();

			for(TrnSeatVO trnSeatVO : trnSeatVOList) {
				seatId+=trnSeatVO.getSeatId();
	...
	public List<TrnSeatVO> getSeat(@RequestBody HashMap<String, String> map){
		...
		String arTm = map.get("arTm");
		String stTm = map.get("stTm");
		arTm=  arTm.replaceAll("(.{2})$", "");
		stTm=  stTm.replaceAll("(.{2})$", "");
		map.replace("arTm",arTm);
		map.replace("stTm",stTm);
	...
	public String payComplete(
        ...
        data.put("trRsvDt", today);
        data.put("payDt", today);
        data.put("trRsvId","T"+now);
        ...
        int countRsvId=this.mmTrnService.countTrRsvId("T"+now);
	    if (countRsvId==0) {
	    	data.put("trRsvId","T"+now+"01");
		    session.setAttribute("trRsvId","T"+now+"01");
	    }else {
        	data.put("trRsvId",mmTrnService.checkTrRsvId("T"+now));
        	session.setAttribute("trRsvId",mmTrnService.checkTrRsvId("T"+now));
	    ...
	    session.setAttribute("rsvTrnNm",data.get("trnKnd")+" "+data.get("trnNum")+" "+data.get("rmNum")+"호차 "+data.get("seatId")+"번 좌석");
	    session.setAttribute("buyerNm",data.get("buyerNm"));
	    session.setAttribute("payTrnPr",data.get("payPr"));
	...
	public  String rsvConfirm(HttpSession session, Model model) {
		...
		String rsvTrnNm = (String) session.getAttribute("rsvTrnNm");
	    String trRsvId = (String) session.getAttribute("trRsvId");
	    String buyerNm = (String) session.getAttribute("buyerNm");
	    String payTrnPr = (String) session.getAttribute("payTrnPr");
