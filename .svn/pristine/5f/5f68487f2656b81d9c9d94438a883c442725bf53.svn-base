...
public class BsMyPageController {
	...
	@PostMapping("/chckInPost")
	@ResponseBody
	public int chckInPost(Model model ,@RequestBody BusiVO busiVO, HttpSession session ) {
		log.info(" chckInPost에 왓다 "); 
		
		BusiVO sBusiVO =  (BusiVO) session.getAttribute("vo");
		model.addAttribute("data", sBusiVO);
		
		String setChckIn  =busiVO.getChckIn();
		String setChckOut  =busiVO.getChckOut();
		log.info("setChckIn: "+setChckIn); 
		
		sBusiVO.setChckIn(setChckIn);
		sBusiVO.setChckOut(setChckOut);
		
		
		log.info("sBusiVO:" +sBusiVO); 
		
		int result =this.bsBusiService.chckIn(sBusiVO);
