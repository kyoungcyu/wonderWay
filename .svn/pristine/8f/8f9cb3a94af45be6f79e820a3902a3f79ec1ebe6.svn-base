...
<div class="page-content-wrapper p-xxl-4">
	...
	<form class="row g-4 align-items-center" id="frm01">
		<div class="col-lg-6">
			<h1 class="h3 mb-3 mb-sm-0">고객 문의</h1>
		</div>
		...
		<div class="col-md-6 col-lg-2">
			<select class="form-select js-choice" aria-label=".form-select-sm" name="sort" id="sort">
				...
				<option value="reprtCk" <c:if test="${param.sort=='reprtCk'}">selected</c:if>>미답변</option>
			</select>
		</div>
	</form>
	<!-- Filters END -->
				...
				<table class="table align-middle p-4 mb-0 table-hover table-shrink">
					...
					<thead class="table-light">
						<tr>
							...
							<th scope="col" class="border-0 text-center">답변 여부</th>
					...
					<c:forEach var="brdVO" items="${data.content}" varStatus="stat">
						...
						<tbody class="border-top-0">
							<!-- Table item -->
							<tr>
								<td> <h6 class="mb-0"><a href="/member/csc/inquiryDetail?brdId=${brdVO.brdId}">${brdVO.brdId}</a></h6> </td>
								<td>
									<h6 class="mb-0"><a href="/member/csc/inquiryDetail?brdId=${brdVO.brdId}">${brdVO.brdTt}</a></h6>
								</td>
								<td><fmt:formatDate value="${brdVO.frstDt}" pattern="yyyy-MM-dd" /></td>
								<td><fmt:formatDate value="${brdVO.lastDt}" pattern="yyyy-MM-dd" /></td>
								<td class="text-center">
									<div class="badge ${brdVO.reprtCk == 'Y' ? 'bg-success text-success' : 'bg-danger text-danger'} bg-opacity-10">
									    ${brdVO.reprtCk == 'Y' ? '답변완료' : '미답변'}
									</div>
								</td>
								<td class="text-center">${brdVO.brdDel}</td>
								<td>
									<div><h6 class="mb-0"><a href="/admin/mem/detail?memId=${brdVO.frstWrtr}">${brdVO.frstWrtr}</a></h6></div>
								</td>
							</tr>
						</tbody>
						<!-- Table body END -->
