					...
					<a class="avatar avatar-sm p-0" href="#" id="profileDropdown" role="button" data-bs-auto-close="outside" data-bs-display="static" data-bs-toggle="dropdown" aria-expanded="false">
						<c:choose>							
								<c:when test="<%=memVO1.getMemImg() == null%>">
										<img class="avatar-img rounded-2" src="/resources/upload/anonymous.jpg" alt="avatar">
								</c:when>
								<c:otherwise>
...
<!-- 							MemVO(memId=test01, memPw=1234, memNm=홍길동, memEnm=GillDong, memFmnm=Hong, memGen=M
							...
							<li class="px-3 mb-3">
								<div class="d-flex align-items-center">
									...
									<c:choose>							
								<c:when test="<%=memVO1.getMemImg() == null%>">
										<img class="avatar-img rounded-circle shadow" src="/resources/upload/anonymous.jpg" alt="avatar">
								</c:when>
								<c:otherwise>
