...
<body>
	...
	<div class="container vstack gap-4">
		...
		<div class="row">
			<div class="col-12">
				<h1 class="fs-4 mb-0">
					<i class="bi bi-star fa-fw me-1"></i>리뷰
				</h1>
			</div>
		</div>
		<!-- Title END -->
			...
			<div class="col-12">
				<div class="card border h-100">
					...
					<div class="card-header border-bottom">
						<h5 class="card-header-title">리뷰 현황</h5>
					</div>
					...
					<div class="card-body">
						<div class="row g-4">
							...
							<div class="col-sm-6 col-xl-3">
								<div class="d-flex align-items-center">
									<!-- Icon -->
									<span class="display-6 text-warning"> <i
										class="bi bi-star"></i>
									</span>
									<!-- Content -->
									<div class="ms-3">
										<h4 class="mb-0" id="averageStar">${averageStar}</h4>
										<span>평균별점</span><br /> <span id="myAnnotation">블라인드리뷰
											별점반영</span>
									</div>
								</div>
							</div>
							...
							<div class="col-sm-6 col-xl-3">
								<div class="d-flex align-items-center">
									<!-- Icon -->
									<span class="display-6 text-danger"> <i
										class="bi bi-exclamation-octagon"></i>
									</span>
									<!-- Content -->
									<div class="ms-3">
										<h4 class="mb-0" id="totalBlind">${totalBlind}</h4>

										<span>블라인드 리뷰 수</span><br/><span>
										<a href="/business/review/blind"> <i
											class="bi bi-arrow-right-circle"></i>블라인드 리뷰만 보기
										</a></span>
							...
							<div class="col-sm-6 col-xl-3">
								<div class="d-flex align-items-center">
									...
									<span class="display-6 text-success"> <i
										class="bi bi-bookmark-star"></i>
									</span>
									<!-- Content -->
									<div class="ms-3">
										<h4 class="mb-0">
											<span id="cmtReview">${totalCmt}</span>/${totalRvw}
										</h4>

										<span>답글 단 리뷰 수</span><br/>
										<span><a href="/business/review/noCmt"> <i
												class="bi bi-arrow-right-circle"></i>노코멘트 리뷰만 보기
										</a></span>
		...
		<div class="row">
			<div class="col-12">
				<div class="card border rounded-3">
					<!-- Card header START -->
					<div class="card-header border-bottom">
						<h5 class="card-header-title">사용자 리뷰</h5>
					</div>
					<!-- Card header END -->
						...
						<c:forEach var="rvwVO" items="${rvwList.content}"
								...
								<div class="d-sm-flex justify-content-between">
									...
									<div class="d-sm-flex align-items-center mb-3">
										<img class="avatar avatar-md rounded-circle float-start me-3"
											src="/resources/upload/mem${rvwVO.memImg}" alt="avatar">
										<!-- Title -->
										<div>
										...
										<c:forEach var="attachDeVOList"
											...
											<div class="col-4 col-sm-3 col-lg-2">
												<a href="assets/images/category/hotel/4by3/07.jpg"
													...
													src="/resources/upload/rvw${attachDeVOList.savePath}"
													class="rounded"
													alt="/resources/upload/rvw${attachDeVOList.savePath}">
												</a>
											</div>
										</c:forEach>
									</c:if>
							...
							<script type="text/javascript">
								$(document).ready(function(){
									var blindAt = "${rvwVO.rvwBlind}";
									if (blindAt == "Y"){
										fn_blind_onload(${stat1.count});
									}
								});
							</script>
						</c:forEach>
...
function fn_blind_off(statCount, object) {
	...
	$("#backSide"+statCount).attr("hidden","hidden");
	var rsvId = "";
	if($("#rsvId"+statCount).html() != null){
		rsvId = $("#rsvId"+statCount).html();
	} else{
