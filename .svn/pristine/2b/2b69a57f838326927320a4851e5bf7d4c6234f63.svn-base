...
<style>
  ...
  .tiny-slider-inner img {
    width: 100%;
    height: auto; /* 가로 비율에 맞게 높이를 자동으로 조정 */
    display: block; /* 이미지 간격 제거를 위해 블록 레벨 요소로 표시 */
  }
</style>
<body>
<main>
...
<section class="pt-0">
	<div class="container">
		...
		<div class="rounded-3 p-3 p-sm-5" style="background-image: url(/resources/booking/assets/images/category/cab/4by3/02.jpg); 
					background-position: center center; background-repeat: no-repeat; background-size: cover; ">
			<!-- Banner title -->
			<div class="row my-2 my-xl-5"> 
				<div class="col-md-8 mx-auto"> 
					<h1 class="text-center text-white">렌터카 관리 </h1>
				</div>
			...
			<div class="row">
				...
				<div class="col-xl-12 col-xxl-12">
					<div class="vstack gap-4">
						<!-- Card item START -->
						<c:forEach var="carVO"  items="${data.content}" varStatus="stat">
							...
							<div class="col-md-5 position-relative">
								...
								<div class="tiny-slider arrow-round arrow-xs arrow-dark overflow-hidden rounded-2" style="margin-top: 50px; margin-bottom: 10px;">
									<div class="tiny-slider-inner" data-autoplay="false" data-arrow="true" data-dots="false" data-items="1">
										<!-- Image item -->
										<div><img src="${carVO.carImg}" alt="Card image" ></div>
								...
								<div class="col-md-7">
									<div class="card-body py-md-2 d-flex flex-column h-100 position-relative" >
										...
										<div class="card-title mb-1 border-bottom">
											<h4 class="card-title mb-1"> &nbsp; ${carVO.carKnd} </h4>
										</div>
										<small><i class="fas fa-car" >&nbsp;${carVO.carCode}</i>
										<i class="bi bi-fuel-pump-fill" >&nbsp;${carVO.fuelTy}</i>
										<i class="bi bi-person-circle" >&nbsp;${carVO.maxPp}</i></small>
										<br>
										<h7 class="card-title mb-1">
										    <span style="color: green;"></span> ${carVO.carYr}년형  
										</h7>
										<h7 class="card-title mb-1">
										    <span style="color: green;">차량 옵션 :</span> ${carVO.carOp}
										</h7>
										
										<!-- Amenities -->
										<ul class="nav nav-divider mt-3" style="margin-top: 1em;">
<%-- 										  <c:forEach var="cvfcVO" items="${bsRmVO.cvcfcVOList}" varStatus="status"> --%>
												...
												<div
													...
													<div class="d-flex align-items-center">
														<h5 class="fw-bold mb-0 me-1">
															&nbsp;
															<fmt:formatNumber value="${carVO.carFee}"
																pattern="#,###,###" />
														</h5>
														<span class="mb-0 me-2">원/ Hour</span>
													</div>
														<div class="row mt-3 mt-sm-0">
															<div class="col">
																<form action="/rntCar/abCar/detailCar" method="get" class="d-flex">
						...
						<nav class="mb-sm-0 d-flex justify-content-center" aria-label="navigation">
							<ul class="pagination pagination-sm pagination-primary-soft mb-0 ">
								<li class="page-item <c:if test='${data.startPage < 6}'>disabled</c:if>" id="dataTable_previous">
									<a class="page-link" href="/rntCar/abCar/carList?currentPage=${data.startPage-5}&keyword=${param.keyword}&sort=${param.sort}&period=${param.period}" aria-controls="dataTable" data-dt-idx="0"  tabindex="-1">Prev</a>
								</li>
								<c:forEach var="pNo" begin="${data.startPage}" end="${data.endPage}">
									<li class="page-item <c:if test='${param.currentPage eq pNo}'>active</c:if>">
										<a href="/rntCar/abCar/carList?currentPage=${pNo}&keyword=${param.keyword}&sort=${param.sort}&period=${param.period}" aria-controls="dataTable" data-dt-idx="${pNo}" tabindex="0" class="page-link">${pNo}</a>
									</li>
								</c:forEach>
								<li class="page-item <c:if test='${data.endPage == data.totalPages}'>disabled</c:if>" id="dataTable_next">
									<a class="page-link" href="/rntCar/abCar/carList?currentPage=${data.startPage-5}&keyword=${param.keyword}&sort=${param.sort}&period=${param.period}" aria-controls="dataTable" data-dt-idx="7"  tabindex="-1">Next</a>
								</li>
							</ul>
						</nav>
					</div>
  ...
  <div class="modal-dialog" role="document">
    <div class="modal-content">
      <div class="modal-header">
        <h5 class="modal-title" id="exampleModalLabel">차량 삭제하기</h5>
        <button type="button" class="close" data-dismiss="modal" aria-label="Close">
          <span aria-hidden="true">&times;</span>
        </button>
      </div>
      <div class="modal-body">
        <h6>해당 상품을 <span style="color: red;">삭제</span> 하시겠습니까?</h6>
      </div>
